...
- `VERSION`, 플러그인·개별 스킬 버전과 태그는 배포 묶음이 확정될 때 한 번에 올립니다. 현재 기준선 다음 일반 배포의 기본 후보는 `v0.7.1`입니다.
...
- 공고문·RFP·공식 양식·근거자료와 서로 다른 제출 사건의 서명·접수 증거는 자동 이동하지 않습니다. 버전이 모호하거나 검증에 실패해도 기존 현재본을 유지합니다.
- 이 보관함은 실제 프로젝트 폴더에 생성됩니다. GitHub Release ZIP을 공유 Drive의 `latest/archive`로 나누는 배포 운영과는 별개입니다.
