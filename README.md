...
AX1 팀의 사업계획서와 보고·제안 발표자료 등 반복 산출물 업무를 위한 통합 스킬팩 배포 저장소입니다. 실제 공고문, RFP, 사업계획서 원본과 생성 산출물은 이 저장소에 넣지 않고 접근이 통제된 공유 드라이브에서 관리합니다.

기존 저장소명과 7개 `bizplan-*` 호출은 유지하면서 AX1 통합 스킬팩으로 확장하고 있습니다. 첫 범용 확장 스킬인 `ax1-presentation`은 보고·제안·사업 발표자료의 스토리보드와 PPTX 제작을 담당합니다.

현재 작업본은 `ax1-budget`을 추가한 9개 스킬의 차기 배포 준비본입니다. 안정 Release v0.9.4에는 기존 8개만 포함되므로 설치할 때는 선택한 Release의 실제 목록을 기준으로 합니다. 아직 새 버전을 배포한 것은 아닙니다.
...
- `VERSION`, 플러그인·개별 스킬 버전과 태그는 배포 묶음이 확정될 때 한 번에 올립니다. 현재 안정 버전은 `v0.9.4`이며 다음 호환 개선 묶음은 기본적으로 패치 후보로 둡니다.
