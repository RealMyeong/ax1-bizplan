...
- 임의·공식 HWPX 편집 엔진은 upstream 플러그인을 별도 의존성으로 유지합니다. 저장소 안의 표준 라이브러리 경량 모드는 매니페스트 SHA-256으로 승인된 단일 AX1 템플릿 생성에만 사용합니다.
...
- 경량 HWPX 변경: 승인 템플릿 SHA-256, 단일 섹션 fail-closed, 본문·표 셀 160%, 체크박스 의미 보존, 원자 저장과 표준 라이브러리 수용 테스트를 함께 검증
...
| 변경 | 권장 버전 |
|---|---|
| 기존 스킬의 질문·출력·템플릿·폴더·체크리스트·안전·검증 개선, 오류 수정, 호환 가능한 워크플로 보완 | 패치: `0.8.0 → 0.8.1` |
...
마이너 버전은 관리자가 `새 기능군 또는 사용법 전환`이라고 명시적으로 승인한 경우에만 선택합니다. 현재 안정 버전 v0.8.0 이후의 호환 개선 묶음도 기본적으로 패치 후보로 둡니다.
