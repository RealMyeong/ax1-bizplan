...
7개 스킬은 새 작업 요청에서 공통 `사용자 이해 확인` 게이트를 가장 먼저 실행해야 합니다. 최초 응답은 대화에 이미 나온 정보만으로 목적·예상 입력·범위·산출물·가정을 설명하고 끝나야 하며, 사용자의 별도 동의 전에는 읽기 전용 조회를 포함한 도구·파일·외부 작업이 없어야 합니다. 설명이 수정되거나 작업 중 목적·대상·산출물이 실질적으로 달라지면 다시 설명하고 재확인합니다. 이 공통 규칙은 `shared/core/12-user-confirmation-gate.md`를 기준 원본으로 유지하고 빌드에서 각 스킬 참조 폴더로 동기화합니다.
...
| 변경 | 권장 버전 |
|---|---|
| 기존 스킬의 질문·출력·템플릿·폴더·체크리스트·안전·검증 개선, 오류 수정, 호환 가능한 워크플로 보완 | 패치: `0.7.2 → 0.7.3` |
...
마이너 버전은 관리자가 `새 기능군 또는 사용법 전환`이라고 명시적으로 승인한 경우에만 선택합니다. 현재 안정 버전 v0.7.2 이후의 호환 개선 묶음도 기본적으로 패치 후보로 둡니다.
...
- 7개 스킬 모두 첫 응답에서 파일·도구를 사용하지 않고 이해 내용을 설명한 뒤 멈추는지, 사용자가 다음 메시지에서 동의한 뒤에만 탐색을 시작하는지
