...
- 공통 규칙 변경: 9개 스킬에 미치는 영향을 확인하고 필요한 복사본을 함께 갱신
- 스킬 백업 정책: `shared/core/13-skill-backup-retention.md`를 모든 배포 스킬에 동기화. 성공 후 직전 설치본 한 묶음 유지, 실패·동일본 재설치 시 회전 금지, 출처 불명·로컬 고유 수정·산출물 이력 제외를 확인. 실제 정리는 별도 설치·업데이트 요청에서만 실행
- 사업비 양식: `ax1-budget`의 빈 입력영역·SHA-256·4개 시트·2개 표·차트·드롭다운과 원본 대비 수식·서식 보존 검사. 원본 XLSX와 내부 용어사전 DOCX는 배포하지 않고 공통화한 양식 하나만 허용
- 파일명 정책: 용어사전 `2026-09-03 / v0.3` 스냅샷, 신규 사업비 `BUD`, 내부안·승인본 버전 구분과 기존 파일의 다음 정식 개정 전환 확인
- HWPX 규칙·버전 변경: 익명화된 HWPX로 생성, 사본 편집, readback, 프리뷰와 Windows 한컴 재개방까지 검증
- 경량 HWPX 변경: 승인 템플릿 SHA-256, 단일 섹션 fail-closed, 본문·표 셀 160%, 표 1행·1열 중앙·나머지 셀 왼쪽 맞춤, 체크박스 의미 보존, 개정 이력·파일명 버전 일치, 기존 출력 무덮어쓰기, 원자 저장과 표준 라이브러리 수용 테스트를 함께 검증
- HWPX 경로 변경: 사업계획서 공식 양식과 AX1 일반 산출물 템플릿을 서로 대체하지 않는지 검증

9개 스킬은 새 작업 요청에서 공통 `사용자 이해 확인` 게이트를 가장 먼저 실행해야 합니다. 최초 응답은 대화에 이미 나온 정보만으로 목적·예상 입력·범위·산출물·가정을 설명하고 끝나야 하며, 사용자의 별도 동의 전에는 읽기 전용 조회를 포함한 도구·파일·외부 작업이 없어야 합니다. 설명이 수정되거나 작업 중 목적·대상·산출물이 실질적으로 달라지면 다시 설명하고 재확인합니다. 이 공통 규칙은 `shared/core/12-user-confirmation-gate.md`를 기준 원본으로 유지하고 빌드에서 각 스킬 참조 폴더로 동기화합니다.
...
| 변경 | 권장 버전 |
|---|---|
| 기존 스킬의 질문·출력·템플릿·폴더·체크리스트·안전·검증 개선, 오류 수정, 호환 가능한 워크플로 보완 | 패치: `0.9.0 → 0.9.1` |
| 새 독립 스킬, 새 필수 외부 의존성·연동, 팀원이 사용법을 다시 익히거나 기존 프로젝트를 이관해야 하는 기본 흐름 전환, 여러 기능을 묶은 뚜렷한 역량 확장 | 마이너: `0.9.x → 0.10.0` |
...
마이너 버전은 관리자가 `새 기능군 또는 사용법 전환`이라고 명시적으로 승인한 경우에만 선택합니다. DXS 표준 파일명은 새 산출물의 필수 사용 흐름을 추가하므로 v0.9.0으로 배포하며, 이후의 호환 개선 묶음은 다시 패치 후보로 둡니다.
...
- 일반 산출물 본문 품질 변경이면 `examples/general-artifact-content-cases.md`로 지침 혼입·근거/불확실성 보존·ID/코드 구별·문서 목적별 예외·수정 후 전역 재검토를 확인하고, 수동 검토·자동 구조 검사·독립 세션 평가를 혼동하지 않는지
- 9개 스킬 모두 첫 응답에서 파일·도구를 사용하지 않고 이해 내용을 설명한 뒤 멈추는지, 사용자가 다음 메시지에서 동의한 뒤에만 탐색을 시작하는지
- `ax1-presentation` 변경이면 스토리보드·모드·이미지형 샘플 게이트, 이미지 번호·화면비·덮어쓰기 거부, 발표자 노트의 실제 한글 보존, PPTX 재개방과 모든 슬라이드 시각 검증을 확인하는지
- `ax1-budget` 변경이면 `python scripts/budget_acceptance_test.py`와 통합·개별 ZIP 격리 검사를 통과하는지, 변경 화면·합성 금액 검증을 수행했는지, 실제 Excel 검증 여부를 구분했는지. 기존 대시보드 5개 비목 범위 밖의 집계를 무조건 정상으로 판정하지 않는지
...
- 경량 생성 결과의 제목이 최대 3단계이고 제목 앞 공백이 수준 1·2·3 순서로 0·3·5개이며 `개요 1`·`개요 2`·`개요 3` 스타일을 연결하는지, 4단계 이상 제목이 앞 공백 없는 본문 목록으로 전환되는지
- 본문 목록의 왼쪽 4000·첫 줄 -800 HWPUNIT hanging indent와 후속 `lineseg` 정렬, 본문·목록·표 뒤 수준 2~3 제목의 1200 HWPUNIT(12pt) 윗간격, 연속 제목·수준 1 새 쪽의 0 간격 예외를 검사하는지
- 본문 표의 1행·1열 전체가 가로 가운데·세로 중앙이고 나머지 셀이 가로 왼쪽인지, 양쪽 정렬 문단모양이 내용 셀에 남지 않는지
...
커밋·병합과 버전 태그는 분리합니다. 개선 요청 한 건이나 커밋 한 건이 끝날 때마다 태그를 만들지 않고, 일요일까지 검증된 변경을 `Unreleased`에 모아 월요일 배포 묶음으로 태그합니다. 검증 완료 변경이 없는 주에는 태그를 만들지 않으며, 업무 중단 오류만 별도 패치로 앞당길 수 있습니다.
...
개별 스킬 ZIP은 GitHub Release에 게시하거나 공유 Drive로 배포하지 않습니다. 팀원은 통합 ZIP의 9개 스킬을 함께 설치하고, 개별 ZIP은 CI 내부 격리 검증 결과로만 사용합니다.
...
업데이트 방법
Codex 또는 Claude에게 "AX1 Skill Pack을 최신 안정 버전으로 업데이트하고 설치 결과를 검증한 뒤 직전 설치 버전 백업만 남겨줘. 산출물 이력과 로컬 수정본은 보존해줘"라고 요청하세요.
