...
| 변경 | 권장 버전 |
|---|---|
| 기존 스킬의 질문·출력·템플릿·폴더·체크리스트·안전·검증 개선, 오류 수정, 호환 가능한 워크플로 보완 | 패치: `0.7.1 → 0.7.2` |
...
마이너 버전은 관리자가 `새 기능군 또는 사용법 전환`이라고 명시적으로 승인한 경우에만 선택합니다. v0.7.1 다음 일반 개선 묶음은 기본적으로 v0.7.2 후보로 둡니다.
