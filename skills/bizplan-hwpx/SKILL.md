...
- 먼저 [문서 유형별 양식 선택](references/12-document-routing.md)을 읽고 `공식 사업계획서 양식`과 `AX1 일반 산출물 양식`을 구분함
...
5. 원본 SHA-256 불변과 출력 파일 분리
6. `render_preview` 페이지 검토
7. Windows 한컴오피스 실제 재개방과 전체 페이지 관찰
8. `bizplan-preflight`의 제출 전 내용·수치·민감정보 점검
